...
- **Weekly Meal Planner** — Mon–Sun calendar with breakfast, lunch, and dinner slots. Assign recipes manually or auto-generate a full week scored against your macro goals and weekly budget.
...
- **Budget & Store Pricing** — Set your zip code and weekly grocery budget. Grocery lists show estimated Walmart prices (via Apify) with USDA fallback; recipe search filters by per-meal cost via Spoonacular.
- **Grocery Lists** — Ingredients from your planned meals are aggregated, categorized, priced, and shown in-app with checkboxes. Download a printable HTML list or get it in your weekly email.
...
| `APIFY_API_TOKEN` | [console.apify.com](https://console.apify.com) — optional, for Walmart grocery price estimates by zip |
...
│   ├── main.py
│   ├── models/models.py
│   ├── routes/          # meals, recipes, goals, profile, scan, files, email, chat, ingest
│   └── services/        # spoonacular, claude, resend, file_gen, grocery, apify_walmart, usda, pricing, agent
├── frontend/
│   └── src/
│       ├── components/  # WeeklyPlanner, GroceryList, BudgetTracker, MacroSummary, RecipeCard, ChatPanel
